...
st.markdown(
    "<h1 style='color:#2b5876;'>💰 Duck San Expense Manager</h1>",
...
if st.button("💾 Save Record", use_container_width=True):
    ...
    st.success("✅ Record Saved to Excel & Supabase!")
    time.sleep(0.5)
    st.rerun()
...
def sync_supabase_to_excel(excel_path):
    try:
        ...
        merged = pd.concat([local_df, supa_data]).drop_duplicates(
            subset=["created_at"], keep="last"
        )
...
with f2:
    cat_filter = st.selectbox(
        "📂 Filter by Category", ["All"] + sorted(df["Category"].unique())
    )
...
header_cols = st.columns([1.3, 1.4, 2, 1.3, 1.2, 1.8])
for c, h in zip(header_cols, ["Date", "Category", "Description", "Vendor", "Amount", "Receipt_url"]):
    c.markdown(f"**{h}**")

view_df = view_df.sort_values("Date", ascending=False).reset_index(drop=True)
for _, row in view_df.iterrows():
    cols = st.columns([1.3, 1.4, 2, 1.3, 1.2, 1.8])
    ...
    cols[5].markdown(
        f"[🔗 View]({link})" if str(link).startswith("http") else "-",
        unsafe_allow_html=True,
    )
...
with st.expander("📊 Monthly & Category Summary", expanded=False):
    ...
    with col1:
        month_summary = st.selectbox(
            "📅 Select Month", ["All"] + list(months), key="summary_month"
        )
    with col2:
        cat_summary = st.selectbox(
            "📁 Select Category",
    ...
    if summary_df.empty:
        ...
        display_df = summary_df[
            ["Date", "Category", "Description", "Vendor", "Amount", "Receipt_url"]
        ].copy()
